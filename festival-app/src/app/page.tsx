      ...
      <div className="p-6 max-w-7xl mx-auto">
        <header className="mb-10 pt-6 flex justify-between items-center">
          <div>
            ...
            <p className="font-bold uppercase text-[9px] text-zinc-500 tracking-widest mt-1">{user.name} • SYNCED</p>
          </div>
              ...
              <div className="space-y-2">
                {items.map(it => (
                  <div key={it.id} className="flex justify-between items-center bg-zinc-900 p-3 rounded-xl border border-white/5">
                    ...
                    <button onClick={async () => { await supabase.from('checklist').update({ is_done: !it.is_done }).eq('id', it.id); fetchData(); }} className={`w-6 h-6 rounded-md border-2 transition-all ${it.is_done ? 'bg-green-500 border-green-400' : 'border-white/20'}`}>{it.is_done && "✓"}</button>
                  </div>
                ))}
              </div>
            </div>
...
function MessageWall({ isOpen, onClose, user }: { isOpen: boolean, onClose: () => void, user: UserProfile }) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const timer = setTimeout(() => { fetchWall(); }, 0);
